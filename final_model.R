...
# set model parameters of best model from model_iterations.R
mod <-rand_forest(mtry = 40, trees = 500, min_n = 10) %>%
...
# workflow for default dataframe
set.seed(345)
wf <- workflow() %>%
  add_model(mod) %>%
  add_recipe(recipe)
...
#save model
saveRDS(fit_rs, "ranger_final_model.RDS")

#collect accuracy and roc_auc
collect_metrics(fit_rs) #accuracy .978, roc_auc .988
